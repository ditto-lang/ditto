...
impl TypeDeclaration {
    fn type_name(&self) -> &cst::ProperName {
        match self {
            Self::Type(cst_type_declaration) => cst_type_declaration.type_name(),
            Self::TypeAlias(cst_type_alias_declaration) => &cst_type_alias_declaration.type_name,
        }
    }
}
...
fn check_cyclic_type_declarations(
    ...
    for type_declaration in type_declarations {
        let type_declaration = if let TypeDeclaration::Type(decl) = type_declaration {
            decl
        } else {
            todo!()
        };
