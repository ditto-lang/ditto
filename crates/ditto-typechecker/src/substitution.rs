use crate::{
    scheme::{get_type_variables, SchemeForall},
    supply::Supply,
};
...
pub struct Substitution(halfbrown::HashMap<usize, Type>);
...
impl Substitution {
    ...
    pub fn fresh(vars: SchemeForall, supply: &mut Supply) -> Self {
        let mut hb = halfbrown::HashMap::new();
        for var in vars {
            hb.insert(var, supply.fresh_type());
        }
        Self(hb)
    }
    ...
    fn apply_rec(&self, ast_type: Type, depth: usize) -> Type {
        ...
        if depth > 100 {
            // Panicking like this is nicer than a stack overflow
            panic!(
        ...
        match ast_type {
            ...
            Type::Variable {
                ...
                if let Some(t) = self.0.get(&var).cloned() {
            ...
            Type::Call {
                ...
                for (var, t) in alias_variables.clone().into_iter().zip(arguments.clone()) {
                    if get_type_variables(&t).contains(var) {
    ...
    fn apply_expression(&self, expression: Expression) -> Expression {
        if self.0.is_empty() {
            return expression;
        }
